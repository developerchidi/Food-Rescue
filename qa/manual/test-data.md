...
## 1. Muc dich
Cung cap bo test data dung chung cho Dev/QA de chay lai manual test va regression mot cach nhat quan (dong bo voi `test-plan-create-marketplace-order.md`).
...
Goi y dat ten title de trace trong DB/log: `[QA][Sprint-XX][Case-ID] ...` (vi du `[QA][Sprint-12][TC-API-POST-001] Com ga nuong`).
...
| Case | Expected status | Expected thong diep (goi y) |
|---|---|---|
...
| create post invalid quantity | 400 | So luong it nhat la 1 |
| create post invalid expiry | 400 | Thoi gian het han khong hop le |
| donation valid pickup | 200 | tra ve donation id |
| donation valid delivery | 200 | tra ve donation id |
...
Luu y: backup DB hoac chi chay tren moi truong test/staging truoc khi cleanup.
