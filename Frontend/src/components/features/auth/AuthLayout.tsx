...
export default function AuthLayout({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen flex bg-[#fdfcf8]">
      {/* Cột Trái: Thông điệp & Hình ảnh (Chỉ hiển thị trên desktop) */}
      <div className="hidden lg:flex w-1/2 bg-mint-darker relative p-16 flex-col justify-between overflow-hidden">
        {/* Background Decor */}
        ...
        <Link href="/" className="flex items-center gap-2 text-white relative z-10 transition-transform hover:scale-105 w-fit">
          <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-mint-darker shadow-lg">
            <Leaf size={24} />
          </div>
          <span className="text-2xl font-black tracking-tight">FoodRescue</span>
        </Link>
        ...
        <div className="relative z-10">
          ...
          <p className="text-white/70 text-lg max-w-md leading-relaxed">
            Hợp tác để giảm thiểu lãng phí thực phẩm và mang đến những bữa ăn ý nghĩa cho cộng đồng.
          </p>
        </div>
        ...
        <div className="relative z-10 bg-white/10 backdrop-blur-md p-8 rounded-[2.5rem] border border-white/10">
          <div className="flex gap-1 mb-4">
            {[1, 2, 3, 4, 5].map((i) => (
              <div key={i} className="text-mint-primary">★</div>
            ))}
          </div>
          <p className="text-white text-lg font-medium italic mb-4">
            "Nền tảng tuyệt vời nhất để tôi có thể đóng góp một phần nhỏ bé của mình vào việc bảo vệ hành tinh."
          </p>
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-mint-primary rounded-full" />
            <div>
              <div className="text-white font-bold">Minh Anh</div>
              <div className="text-white/50 text-sm">Cộng tác viên môi trường</div>
            </div>
          </div>
        </div>
      </div>
